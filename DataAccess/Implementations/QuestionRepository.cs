...
namespace FormBuilder.API.DataAccess.Implementations
{
    public class QuestionRepository : IQuestionRepository
    {
        ...
        public Question GetById(string id) => _questions.Find(q => q.Id == id).FirstOrDefault();

        public IEnumerable<Question> GetAll() => _questions.Find(q => true).ToList();

        public void Update(Question question) => _questions.ReplaceOne(q => q.Id == question.Id, question);

        public void Delete(string id) => _questions.DeleteOne(q => q.Id == id);
    }
}
